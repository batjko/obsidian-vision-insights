...
export class OpenAIClient {
  ...
  private getPromptForAction(action: VisionAction): string {
    const prompts: Record<VisionAction, string> = {
      ...
      'extract-meeting-participants': "Extract and list all meeting participants visible in this screenshot. Look for: 1) Participant names (from name tags, labels, or video tiles), 2) Participant avatars or profile pictures, 3) Status indicators (muted, camera on/off, hand raised, etc.), 4) Roles or titles if visible, 5) Host/presenter indicators. Format as a clean bulleted list with names - surround any firstName lastName combinations with wiki brackets, e.g. [[John Doe]] - and any relevant status information. If no participants are clearly identifiable, indicate what meeting interface elements are visible instead.",
      
      'analyze-meeting-content': "Analyze this meeting screenshot comprehensively, focusing on both the shared content and meeting context. Provide: 1) **Shared Content Analysis** - describe any presentations, documents, or screen shares visible, including key points, slide titles, charts, or data, 2) **Meeting Context** - identify the meeting platform (Zoom, Teams, etc.), meeting layout, and participant information, 3) **Key Information** - extract important text, data, or decisions from the shared content, 4) **Meeting Dynamics** - note any visible interactions like chat messages, raised hands, or presenter mode, 5) **Action Items** - identify any explicit tasks, decisions, or next steps mentioned in the content. Structure the response to be useful for meeting notes and follow-up."
    };
